...
from model.predictive_maintenance import PredictiveMaintenanceModel, SequenceGenerator
...
def run_map_view_simulation(config, test_df, engine_motor_mapping, sequence_cols, nb_features):
    ...
    if st.session_state['run_map_simulation']:
        ...
        classes = config.class_labels if config.OUTPUT_TYPE == 'multiclass' and hasattr(config, 'class_labels') else [
        ...
        critical_list_placeholder = st.empty()
        warning_list_placeholder = st.empty()
        ...
        pred_dfs = {engine_id: pd.DataFrame(
            columns=['Cycle', 'Predicted_Probability'] if config.OUTPUT_TYPE == 'binary' else (
                    ['Cycle'] + [f'Probability_{cls}' for cls in classes]) if config.OUTPUT_TYPE == 'multiclass' else ['Cycle', 'Predicted_Value'])
            for engine_id in engine_ids}
        ...
        pred_dfs = {engine_id: pd.DataFrame(
            columns=['Cycle', 'Predicted_Probability'] if config.OUTPUT_TYPE == 'binary' else (
                    ['Cycle'] + [f'Probability_{cls}' for cls in
                                 classes]) if config.OUTPUT_TYPE == 'multiclass' else ['Cycle', 'Predicted_Value'])
            for engine_id in selected_ids}
                ...
                if engine_cycles_remaining[engine_id] > 0:
                    ...
                    try:
                        ...
                        if config.OUTPUT_TYPE == 'binary':
                            ...
                            st.error(f"Unsupported OUTPUT_TYPE: {config.OUTPUT_TYPE}")
                            continue
                    except Exception as e:
...
def process_engine_prediction(engine_id, cycle_idx, engine_data, placeholders, config,
    ...
    # Extract predicted values based on OUTPUT_TYPE
    if config.OUTPUT_TYPE == 'binary':
    ...
    if config.OUTPUT_TYPE == 'binary':
        ...
        status_indicator = config.STATUS_COLORS.get('safe', '🟢') if y_pred_prob * 100 < threshold else config.STATUS_COLORS.get('critical', '🔴')
    elif config.OUTPUT_TYPE == 'multiclass':
        y_pred_class = np.argmax(y_pred_probs)
        ...
        status_indicator = config.STATUS_COLORS.get('safe', '🟢') if y_pred_value < threshold_value else config.STATUS_COLORS.get('critical', '🔴')
    ...
    if selected_features:
        ...
        placeholders['sensor_placeholder'].plotly_chart(fig_sensor, use_container_width=True, key=f"sensor_{unique_key}")

    # Update Gauge or Display Based on Output Type
    if config.OUTPUT_TYPE == 'binary' and y_pred_prob is not None:
        fig_gauge = go.Figure(go.Indicator(
            mode="gauge+number",
    ...
    elif config.OUTPUT_TYPE == 'multiclass' and y_pred_probs is not None:
        ...
        try:
            ...
            print(unique_key)
    elif config.OUTPUT_TYPE == 'regression' and y_pred_value is not None:
        placeholders['gauge_placeholder'].write(f"Predicted Value at Cycle {cycle_num}: {y_pred_value:.2f}", key=f"value_{unique_key}")
    ...
    if pred_df is not None and not pred_df.empty:
        if config.OUTPUT_TYPE == 'binary':
            ...
            fig_pred.update_yaxes(title_text='Predicted Probability (%)')
            fig_pred.update_xaxes(title_text='Cycle')
            placeholders['prediction_placeholder'].plotly_chart(fig_pred, use_container_width=True, key=f"prediction_{unique_key}")
            ...
            fig_pred = px.line(pred_df, x='Cycle', y='Predicted_Value', title=f"Predicted Values Over Cycles for Engine {engine_id}", height=250)
            fig_pred.update_yaxes(title_text='Predicted Value')
            fig_pred.update_xaxes(title_text='Cycle')
            placeholders['prediction_placeholder'].plotly_chart(fig_pred, use_container_width=True, key=f"prediction_{unique_key}")
